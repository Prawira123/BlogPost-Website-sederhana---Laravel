    ...
    <div class="flex items-center justify-between w-full p-20">
        ...
        <div class="flex flex-col items-start justify-beetwen w-full h-auto p-2 gap-5">
            <div class="flex items-center justify-beetwen w-full h-auto ">
                ...
                <a href="" class="w-[20%]"><button type="submit" class="text-sm/6 font-semibold text-white border-white border-2 rounded-full px-5 py-3 hover:bg-white/80 cursor-pointer hover:text-black p-10 transition-all duration-300">Edit Profile<span aria-hidden="true">&rarr;</span></button></a>
            </div> 
    ...
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div class="mx-auto mt-10 grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 border-t border-gray-700 pt-10 sm:mt-16 sm:pt-16 lg:mx-0 lg:max-w-none lg:grid-cols-3">
            @foreach ($posts as $post)
                <x-card-post :title="$post->title" :content="$post->content" :date="$post->created_at" :img="$post->img" :name="$post->user->name" :category="$post->category->name" ></x-card-post>
            @endforeach
        </div>
    </div>
    </div>
