...
@section('content')
    ...
    @foreach ($comments as $comment )
            <x-detail-post.comment-list :imageProfile="$comment->user->imageProfile" :username="$comment->user->name" :date="$comment->created_at->diffForHumans(['parts' => 2])" :content="$comment->content"></x-detail-post.comment-list>
    @endforeach
@endsection
